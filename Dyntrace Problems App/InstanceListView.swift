...
extension UTType {
    static let o11yConfig = UTType(exportedAs: "seberino.Oyy1Notification.dypc")
}
...
struct InstanceListView: View {
    ...
    var body: some View {
        NavigationStack {
            Group {
                if store.instances.isEmpty {
                    ContentUnavailableView {
                        ...
                        Text("Adicione uma instância do Dynatrace ou do Grafana para começar a monitorar alertas.")
                    } actions: {
            ...
            .fileImporter(isPresented: $showingImport,
                          allowedContentTypes: [.o11yConfig, .data, .propertyList],
                          allowsMultipleSelection: false) { result in
                handleImport(result: result)
            }
            .onOpenURL { url in
                importFile(at: url, isSecurityScoped: true)
            }
    ...
    private func importFile(at url: URL, isSecurityScoped: Bool) {
        ...
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    ...
                    Text(instance.vendor.displayName)
                        .font(.caption2.bold())
                        .padding(.horizontal, 6)
                        .padding(.vertical, 1)
                        .background(vendorColor(instance.vendor).opacity(0.15))
                        .foregroundStyle(vendorColor(instance.vendor))
                        .clipShape(Capsule())
                }
    ...
    private func filterSummary(for instance: DynatraceInstance) -> String? {
        ...
        case .grafana:
            ...
            if !matchers.isEmpty { parts.append("labels: \(matchers.count)") }
        }
    ...
    private func refresh(instance: DynatraceInstance) async {
        ...
        do {
            ...
            let displayCount = aggregate.totalCount ?? aggregate.items.count
            problemCounts[instance.id] = .loaded(displayCount)
            await NotificationManager.shared.processFetchedAlerts(
                aggregate.items, for: instance, triggerNotifications: false)
        } catch {
