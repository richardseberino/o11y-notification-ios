...
struct InstanceEditView: View {
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("URL (ex: https://abc12345.live.dynatrace.com)", text: $instance.baseURL)
                        .keyboardType(.URL)
                        .textInputAutocapitalization(.never)
                        .autocorrectionDisabled()
                    SecureField("API Token", text: $instance.apiToken)
                        .textInputAutocapitalization(.never)
                        .autocorrectionDisabled()
                } header: {
                    Text("Instância")
                } footer: {
                    Text("O token precisa do escopo \"Read problems\". Para listar as Management Zones, inclua também \"Read configuration\".")
                }
            ...
            .task {
                ...
                if canFetchOptions && !hasFetchedZones {
                    await reloadOptions()
                }
            }
        }
    ...
    private func reloadOptions() async {
        guard canFetchOptions else { return }
        loadingZones = true
        zonesError = nil
        do {
