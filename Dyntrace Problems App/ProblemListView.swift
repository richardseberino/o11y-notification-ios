...
struct ProblemListView: View {
    ...
    @State private var problems: [DynatraceProblem] = []
    ...
    private var filteredProblems: [DynatraceProblem] {
        ...
        return problems.filter { problem in
            ...
            problem.affectedResource.lowercased().contains(query)
        }
    }
    ...
    @ViewBuilder
    private func problemRow(_ problem: DynatraceProblem) -> some View {
        HStack(alignment: .top, spacing: 10) {
            Circle()
                .fill(severityColor(problem.severityLevel))
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(formattedDate(problem.startDate))
                    if let displayId = problem.displayId {
                        Text("·")
                        Text(displayId)
                    }
                }
    ...
    private func severityColor(_ severity: String?) -> Color {
        ...
        case "AVAILABILITY", "ERROR": return .red
        case "PERFORMANCE", "RESOURCE_CONTENTION", "RESOURCE": return .orange
    ...
    private func fetchMore() async {
        ...
        var collected: [DynatraceProblem] = []

        do {
            while collected.count < pageSize {
                let page = try await DynatraceAPI.fetchProblemsPage(for: instance, nextPageKey: key)
                collected.append(contentsOf: page.problems)
